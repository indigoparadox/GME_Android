...
class MusicEmu {
    ...
    protected PlayMusic timeMovementReceptor = null;
    ...
    // Requests change of sample rate and returns sample rate used, which might
    // be different
     ...
     * 
     * @param count
     *            The number of samples to fill with.
     */
	...
	if( !trackEnded_ ) {
		...
		applyFade( out, count );
	    timeMovementReceptor.setDisplayTime( currentTime_ );
	} else {
     ...
     */
    public final void setTimeMovementReceptor( PlayMusic receptorIn ) {
	timeMovementReceptor = receptorIn;
    }
    ...
    static final int fadeShift = 8; // fade ends with gain at 1.0 / (1 <<
	...
	for( int i = 0; i < count; i += fadeBlockSize ) {
	    ...
	    int gain = int_log( (currentTime_ + i - fadeStart) / fadeBlockSize,
			fadeStep, gainUnit );
